...
def get_opportunity_activities(name):
	...
	doc = frappe.db.get_values("Opportunity", name, ["creation", "owner", "opportunity_from", "party_name"])[
		0
	]
	...
	if opportunity_from == "Lead":
		lead = doc[3]
		activities, calls, _notes, tasks, events, attachments, _opportunities = get_lead_activities(
			lead, False, True
		)
...
def get_lead_activities(name, get_events=True, exclude_crm_note_attachments=False):
	...
	return activities, calls, notes, tasks, events, attachments, opportunities
...
def delete_attachment(filename, doctype=None, docname=None):
	...
	if doctype and docname:
		...
		for note in notes:
			...
			updated_attachments = [
				row for row in note_doc.custom_note_attachments if row.filename != filename
			]
