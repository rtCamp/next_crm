...
Mirrors https://github.com/frappe/erpnext/pull/55022 — shipped here so sites
running next_crm don't have to wait on the upstream patch landing.

The v15 patch `erpnext.patches.v15_0.migrate_to_utm_analytics` migrated the
`Lead Source` lookup doctype's rows into `UTM Source`, then renamed the
per-row `source` field on Lead/Opportunity to `utm_source` via the
`oldfieldname` schema hint.

That hint is documentation-only in Frappe — the only code path that actually
issues a `RENAME COLUMN` is the desk-driven Custom Field rename flow.
Standard DocField schema-sync only adds new columns and modifies existing
ones; it never renames or drops based on JSON changes. So on every site
that ran the v15 patch, `utm_source` was added empty alongside the legacy
`source` column, and every row's source value was orphaned on the now
meta-less `source` column.
