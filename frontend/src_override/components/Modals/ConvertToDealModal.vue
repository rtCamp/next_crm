...
async function convertToDeal() {
  ...
  if (existingOrganizationChecked.value && !existingOrganization.value) {
    error.value = __('Please select an existing customer')
    return
  }
  ...
  let _deal = await call('next_crm.overrides.lead.convert_to_opportunity', {
    lead: props.lead.name,
    opportunity: deal.doc,
...
const dealTabs = createResource({
  ...
  cache: ['RequiredFields', 'Opportunity'],
  params: { doctype: 'Opportunity', type: 'Required Fields' },
  auto: true,
  transform: (_tabs) => {
    let hasFields = false
