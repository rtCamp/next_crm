<template>
  <div v-if="show" class="flex flex-col w-[352px] text-base h-full">
    ...
    <div v-if="mode == 'details'" class="flex flex-col flex-1 overflow-y-auto">
      ...
      >
        <component
          :is="_event.referenceDoctype == 'Lead' ? LeadsIcon : DealsIcon"
          class="size-4"
        />
        <Link
        ...
        >
          ...
          <div class="flex items-center gap-x-1.5 w-full">
            <FormControl
              ...
              variant="outline"
              :placeholder="__('Add lead or opportunity')"
              @change="
                () => {
        ...
        >
          ...
          <div class="flex items-center gap-x-1.5 w-full">
            <Link
              ...
              :doctype="_event.referenceDoctype"
              :filters="
                _event.referenceDoctype === 'Lead' ? { custom_converted: 0 } : {}
              "
              variant="outline"
...
function redirect() {
  if (_event.value.referenceDocname) {
    let name = _event.value.referenceDoctype === 'Lead' ? 'Lead' : 'Opportunity'

    let params =
      _event.value.referenceDoctype == 'Lead'
