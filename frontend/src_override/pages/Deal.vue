<template>
  <LayoutHeader>
    ...
    <template v-if="!errorTitle" #right-header>
      ...
      />
      <AssignTo v-model="assignees.data" doctype="Opportunity" :docname="dealId" />
      <Dropdown
        v-if="doc"
        :options="statuses"
        placement="right"
...
const { doctypeMeta } = getMeta('Opportunity')
...
const { triggerOnChange, assignees, permissions, document, scripts, error } =
  useDocument('Opportunity', props.dealId)
...
const opportunityFrom = reactive({
  opportunityFrom: doc?.data?.opportunity_from,
  partyName: doc?.data?.party_name,
})
...
const breadcrumbs = computed(() => {
  ...
  items.push({
    label: title.value,
    route: { name: 'Opportunities', params: { dealId: props.dealId } },
  })
  return items
})

const title = computed(() => {
  let t = doctypeMeta['Opportunity']?.title_field || 'name'
  return doc.value?.[t] || props.dealId
})

const statuses = computed(() => {
  return statusOptions('deal', [], triggerStatusChange)
})
...
const dealContacts = createResource({
  url: 'next_crm.api.contact.get_lead_opportunity_contacts',
  params: {
    doctype: "Opportunity",
    docname: props.dealId
  },
...
function updateField(name, value) {
  ...
  // if (name == 'opportunity_from' || name == 'party_name') {
  //   name == 'opportunity_from' ? opportunityFrom.opportunityFrom = value : opportunityFrom.partyName = value
...
function beforeStatusChange(data) {
  // if (data?.hasOwnProperty('opportunity_from') || data?.hasOwnProperty('party_name')) {
  //   data?.hasOwnProperty('opportunity_from') ? opportunityFrom.opportunityFrom = data.opportunity_from : opportunityFrom.partyName = data.party_name
  //   showOpportunityFromModal.value = true
  //   return
  // }
  if (
