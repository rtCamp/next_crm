              ...
              <div class="flex gap-4 items-center">
                <div class="group relative h-15.5 w-15.5">
                  <Avatar
                    size="3xl"
                    class="h-15.5 w-15.5"
                    :label="organization.doc.customer_name"
                    :image="organization.doc.image"
                  />
                  <component
...
const { doctypeMeta } = getMeta('Customer')
...
const {
  ...
  permissions,
  scripts,
} = useDocument('Customer', props.organizationId)
...
const title = computed(() => {
  let t = doctypeMeta['Customer']?.title_field || 'name'
...
function beforeFieldChange(data) {
  if (data?.hasOwnProperty('customer_name')) {
    call('frappe.client.rename_doc', {
      ...
      router.push({
        name: 'Customer',
        params: { organizationId: data.customer_name },
      })
    })
  } else {
...
const sections = createResource({
  ...
  cache: ['sidePanelSections', 'Customer'],
